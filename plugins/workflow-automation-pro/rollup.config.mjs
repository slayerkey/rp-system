...
const sdPlugin = "com.packrat.workflowpro.sdPlugin";
					...
					if (error.code === "EPERM" || error.code === "EBUSY") {
						this.warn(
							`Could not refresh ${dest} -- the plugin is running and holding it open. ` +
								`Existing copy left in place. Run 'streamdeck stop com.packrat.workflowpro' first if koffi changed.`
						);
						continue;
					}
					throw error;
...
const config = {
	...
	output: {
		// ../_wf/src/input/index.ts dynamically imports whichever platform module matches
		// process.platform, so the other one's module-scope native library load (user32.dll on
		// Windows, CoreGraphics on macOS) never runs on the wrong OS. That only holds if
		// each platform module stays a SEPARATE chunk whose body is evaluated lazily at the
		// import() call site. inlineDynamicImports would flatten them into the entry and
		// evaluate them eagerly at startup, which crashes the plugin on load. So emit a dir
		// with real chunks; the entry stays bin/plugin.js per the manifest.
		dir: `${sdPlugin}/bin`,
	...
	plugins: [
		...
		},
		{
			...
			buildStart() {
				// koffi's loader throws "Mismatched native Koffi modules" at runtime if the core
				// package.json version doesn't match whatever native was actually installed --
				// a plugin can pass this existence check and still be dead on the platform whose
				// native drifted from core, as shipped 2026-08-14 (koffi 3.1.5 core / darwin 3.1.4).
					...
					if (!fs.existsSync(file)) {
						this.error(
							`Missing ${file}. koffi's prebuilt binaries are optional dependencies gated by ` +
								...
								`'npm install --force ${pkg}@${coreVersion}' to fetch the rest.`
						);
					}
					const pkgVersion = JSON.parse(fs.readFileSync(`node_modules/${pkg}/package.json`, "utf8")).version;
					if (pkgVersion !== coreVersion) {
						this.error(
							`${pkg}@${pkgVersion} does not match koffi core@${coreVersion}. This builds fine and ` +
								`throws "Mismatched native Koffi modules" at runtime on that platform. Run ` +
								`'npm install --force ${pkg}@${coreVersion}' to realign it.`
						);
					}
				}
			}
		...
		typescript({
			...
			// The shared core is imported as TypeScript source by relative path. Without it in
			// include, Rollup hands those files to its JS parser and chokes on the type syntax.
			include: ["src/**/*.ts", "../_wf/src/**/*.ts"]
		}),
		nodeResolve({
			browser: false,
			exportConditions: ["node"],
			preferBuiltins: true,
			// _wf has its own node_modules. Without dedupe the bundle ends up with two SDK
			// instances and two Connection singletons, one of which never registers, and every
			// settings call from shared code hangs forever with no error.
