...
configureSport({ league: { sport: "hockey", league: "nhl" }, teams: TEAMS });

@action({ UUID: "com.packrat.nhl-tracker.score" })
class TeamScore extends TeamScoreBase {}

@action({ UUID: "com.packrat.nhl-tracker.standings" })
class Standings extends StandingsBase {}

@action({ UUID: "com.packrat.nhl-tracker.scoreboard" })
