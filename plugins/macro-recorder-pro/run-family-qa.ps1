  ...
  try {
    ...
    try {
      ...
      foreach ($RequiredSuffix in @(
        ...
        if (-not ($EntryNames | Where-Object { $_.EndsWith($RequiredSuffix) })) {
          throw "$Slug exact package is missing $RequiredSuffix"
        }
      }
...
if (-not $SkipArt) {
  foreach ($Slug in @("macro-recorder-lite","macro-recorder-pro")) {
    ...
    $Gallery = @(Get-ChildItem $MediaRoot -File | Where-Object { $_.Name -match '^0[2-5]-.*\.png })
    if ($Gallery.Count -ne 3) { throw "$Slug expected exactly four Marketplace gallery images; found $($Gallery.Count)." }
  }
}
...
Write-Host "  powershell -ExecutionPolicy Bypass -File .\plugins\macro-recorder-pro\run-family-qa.ps1 -ReleaseCandidate"
        ...
        throw "$Slug exact package must not contain internal profile-map JSON."
      }
    }
    finally {
      $Archive.Dispose()
