...
test("Pro property inspector keeps replay setup visible and debuggable",async()=>{
  ...
  assert.match(runtime,/assignNewRecordingToLatestReplayKeys/);
  ...
  assert.match(runtime,/record\.settings\.followLatest\) title = "SAVED"/);
  ...
  assert.match(runtime,/followLatest/);
  assert.match(runtime,/settings: record \? \{ \.\.\.record\.settings/);
  assert.doesNotMatch(runtime,/else if \(saved\) title = "SAVED"/);
