...
const GLYPHS: Record<WirelessKeyKind,string> = {
  ...
  battery: '<rect x="16" y="12" width="37" height="22" rx="4"/><path d="M53 19h5v8h-5"/><path d="M22 18h17v10H22Z" fill="#FFB21E" stroke="none"/>',
...
export function wirelessKeyImage(kind:WirelessKeyKind, rawLines:string[]):string {
  const lines=normalize(rawLines), size=fontSize(lines);
  const text=lines.length===1
    ? '<text x="72" y="130" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="'+size+'" font-weight="800">'+escapeXml(lines[0])+'</text>'
    : lines.map((line,index)=>'<text x="72" y="'+(index===0?111:135)+'" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="'+size+'" font-weight="800">'+escapeXml(line)+'</text>').join("");
  return "data:image/svg+xml;base64,"+Buffer.from(
    '<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144" viewBox="0 0 144 144">'+
    '<rect width="144" height="144" rx="24" fill="#05070A"/>'+
    '<g transform="translate(21 6) scale(1.42)" fill="none" stroke="'+WIRELESS_ACCENT+'" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round">'+GLYPHS[kind]+'</g>'+
    text+'</svg>',"utf8").toString("base64");
}
