...
export async function handlePiProbe(context: string, payload: unknown): Promise<void> {
	...
	switch (probe) {
		...
		case "mics": {
			...
			await streamDeck.ui.sendToPropertyInspector({
				probe: "mics",
				// `roles` lets the picker spell out which devices "Default microphone"
				// resolves to. Windows has two independent defaults and apps disagree about
				// which one they open, so showing the names is the difference between
				// "muted everywhere" and "muted in Discord but not OBS".
				mics: mics.map((d) => ({
					id: d.id,
					label: d.isDefault ? `${d.name} (default)` : d.name,
					roles: [d.isDefault ? "voice apps" : null, d.isDefaultConsole ? "other apps" : null]
						.filter(Boolean)
				}))
			});
			return;
