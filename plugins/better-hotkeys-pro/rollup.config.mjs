...
const sdPlugin = "com.packrat.betterhotkeyspro.sdPlugin";

// One shipped artifact runs on both OSes, so all three native binaries travel together --
// koffi picks the matching one at runtime via process.platform/process.arch (see
// koffi/src/koffi/index.cjs). npm only auto-installs the optional dep matching the host
// machine, so on a Windows dev box the mac ones need an explicit
					...
					if (error.code === "EPERM" || error.code === "EBUSY") {
						this.warn(
							`Could not refresh ${dest} -- the plugin is running and holding it open. ` +
								`Existing copy left in place. Run 'streamdeck stop com.packrat.betterhotkeyspro' first if koffi changed.`
						);
						continue;
					}
					throw error;
...
const config = {
	...
	output: {
		// src/input.ts dynamically imports whichever of win32/input.ts or darwin/input.ts
		// matches process.platform, so the other one's module-scope native-library load
		// (user32.dll on Windows, CoreGraphics on macOS) never runs on the wrong OS.
		// That only holds if each platform module stays a SEPARATE chunk whose body is
		// evaluated lazily at the import() call site. inlineDynamicImports would flatten
		// both into the entry and evaluate them eagerly at startup -- which crashed the
		// plugin on load (the wrong OS's koffi.load throws "module could not be found").
		// So emit a dir with real chunks; the entry stays bin/plugin.js per the manifest.
		dir: `${sdPlugin}/bin`,
