...
const sdPlugin = "com.packrat.code-cost-pro.sdPlugin";

/**
 * Pulls in plugins/_shared/src (the key-image helper the Packrat plugins share. This plugin makes NO network calls: it reads
 * local agent session logs from disk and renders SVG key faces, so the same bundle runs on
 * Windows and macOS with no native code.
 *
...
const config = {
	...
	plugins: [
		...
		typescript({
			...
			include: ["src/**/*.ts", "../code-cost/src/**/*.ts", "../_shared/src/**/*.ts"]
		}),
		nodeResolve({
			browser: false,
