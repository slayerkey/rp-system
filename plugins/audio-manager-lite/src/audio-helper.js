...
function helperPath() {
  return fileURLToPath(new URL("../native/win-x64/PackRat.AudioManagerLite.Helper.exe", import.meta.url));
}
...
export class AudioHelper {
  ...
  async start() {
    ...
    this.starting = Promise.resolve().then(() => {
      ...
      if (!existsSync(executable)) throw new Error("Audio helper is missing. Reinstall Audio Manager Lite.");
  ...
  onExit(error) {
    if (this.lines) { try { this.lines.close(); } catch {} }
    this.lines = null;
    this.child = null;
