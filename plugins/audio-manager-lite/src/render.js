...
const glyph=(kind,color)=>kind==="set-input"
...
export function renderKey(kind="set-output",{device=null,missing=false,offline=false}={}){
  ...
  return data(`<rect width="144" height="144" rx="24" fill="${BG}"/><rect x="6" y="6" width="132" height="132" rx="19" fill="none" stroke="${color}" stroke-width="3" opacity=".62"/>${glyph(kind,color)}${text(121,name,size,color)}`);
}
