...
const sdPlugin = "com.packrat.ultimatesports.sdPlugin";

/**
 * Pulls in plugins/_shared/src (the ESPN client, poller, and key renderer that every Packrat
 * sport tracker uses) by relative import and bundles it into the single plugin.js. There is no
 * native code anywhere in this plugin: data comes from fetch and key faces are SVG strings, so
 * the same bundle runs on Windows and macOS.
 *
