...
// Everything sport specific about this plugin lives in these three lines. The actions, the
// polling, the key faces and the settings protocol are all shared with the other trackers.
configureSport({ league: { sport: "basketball", league: "nba" }, teams: TEAMS });

@action({ UUID: "com.packrat.nba-tracker.score" })
class TeamScore extends TeamScoreBase {}

@action({ UUID: "com.packrat.nba-tracker.standings" })
class Standings extends StandingsBase {}

@action({ UUID: "com.packrat.nba-tracker.scoreboard" })
