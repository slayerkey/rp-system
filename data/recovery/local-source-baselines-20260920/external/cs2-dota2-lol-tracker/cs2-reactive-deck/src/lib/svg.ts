...
/** Approximate per-character advance (in ems) for our bold sans font. Narrow glyphs are cheaper. */
const CHAR_EM: Record<string, number> = { " ": 0.28, "/": 0.33, ".": 0.3, ",": 0.3, "$": 0.56 };

/** Estimate rendered text width (px) for the bold key font at a given size. */
function approxWidth(text: string, size: number): number {
	let ems = 0;
	for (const c of text) ems += CHAR_EM[c] ?? 0.56;
	return ems * size;
}

/**
 * Pick the largest font-size up to `max` such that `text` fits within `maxWidth` px on a key.
 * Lets value text (KDA, money) shrink to stay on-screen instead of clipping at the edges.
 */
export function fitSize(text: string, maxWidth: number, max: number): number {
	const natural = approxWidth(text, max);
	if (natural <= maxWidth) return max;
	return Math.floor((max * maxWidth) / natural);
}
