# coding: utf-8
#!/usr/bin/env python3
# Creator Stats - Upload-ready marketing assets
# Outputs to scripts/output/upload/ (clean folder, separate from dev output)
# Run: py scripts/gen-marketing-upload.py
...
OUT_DIR    = os.path.join(SCRIPT_DIR, "output", "upload")      # <-- separate upload folder
...
MAIN_Y  = TOP_H
MAIN_BOT = H - BOT_H
MAIN_H  = MAIN_BOT - MAIN_Y
BAN_H   = 130
CT_TOP  = MAIN_Y + BAN_H
CT_H    = MAIN_BOT - CT_TOP
...
def gen_styles():
    ...
    grid_w = COLS * CW; ox = (W - grid_w) // 2; oy = content_oy(KD, 40)
...
def gen_milestones():
    ...
    grid_w = COLS * CW; ox = (W - grid_w) // 2; oy = content_oy(KS, 72)
    ...
    state_descs = ["0 to 1K subs","1K to 10K subs","10K to 100K subs","100K to 1M subs"]
...
def gen_delta():
    ...
    grid_w = COLS * CW; ox = (W - grid_w) // 2; oy = content_oy(KD, 44)
...
def gen_platforms():
    ...
    LOGO_SZ = 280; STAT_SZ = 190; STAT_GAP = 16
    stat_row_w = STAT_SZ * 2 + STAT_GAP
    total_h = LOGO_SZ + 28 + STAT_SZ
    logo_y  = CT_TOP + (CT_H - total_h) // 2
    ...
    add_bottom_bar(canvas,W,H,[
        ("people","FOLLOWERS","AND SUBSCRIBERS"),
...
def gen_marketplace_icon():
    ...
    cx_yt, cy_yt = 80, 110; r_yt = 44
    d.ellipse([(cx_yt-r_yt, cy_yt-r_yt),(cx_yt+r_yt, cy_yt+r_yt)], fill=(255,0,0,255))
    d.polygon([(cx_yt-14, cy_yt-18),(cx_yt-14, cy_yt+18),(cx_yt+20, cy_yt)], fill=(255,255,255,255))

    cx_tw, cy_tw = 208, 110; r_tw = 36
    ...
    img.save(os.path.join(OUT_DIR, "icon-288x288.png"))
    print("> icon-288x288.png")


def write_descriptions():
    # Output to upload folder
    txt_upload = """\
...
creator stats stream deck youtube subscribers twitch followers live viewers milestone progress analytics streaming
"""
    out = os.path.join(OUT_DIR, "description.txt")
    with open(out, "w", encoding="utf-8") as f:
        f.write(txt_upload.strip())
    print("> description.txt")

    # Also write docs/descriptions.md in the Kick style
    os.makedirs(DOCS_DIR, exist_ok=True)
    md = """\
# Creator Stats for Stream Deck - Marketplace Listing Copy

## Search line
Real-time YouTube and Twitch creator analytics on your Stream Deck. Subscriber count, live viewers, milestone progress, and 7-day growth trends always visible.

## Tagline
Your numbers. On your desk. Always.

## Description
Creator Stats puts your most important YouTube and Twitch numbers on your Stream Deck keys. Enter your YouTube API key and channel ID, connect your Twitch account, and stats start loading immediately. No third-party apps, no browser tab switching.

Subscribers, live viewers, follower count, total views, upload streak, and milestone progress. Six display styles per key. Short press to cycle styles. Long press to switch metrics.

## Features
- YouTube and Twitch stats together, no extra apps needed
- Six display styles: big number, milestone ring, sparkline trend, delta, streak, and multi-stat overview
- Milestone mode: progress ring to any target with a days-to-goal estimate
- Live mode: viewer count updates every 90 seconds while you stream, switches back to followers when you go offline
- Upload streak: consecutive upload days tracked automatically from your YouTube activity
- Short press cycles display style, long press cycles the metric shown
"""
    md_path = os.path.join(DOCS_DIR, "descriptions.md")
...
if __name__=="__main__":
    ...
    print("YT:    %s" % ("found" if os.path.exists(YT_PNG) else "NOT FOUND"))
    print("TW:    %s" % ("found" if os.path.exists(TW_PNG) else "NOT FOUND"))
    print("Out:   %s\n" % OUT_DIR)
    gen_hero()
    gen_styles()
    gen_milestones()
