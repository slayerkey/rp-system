...
// Variadic in the real API (wheel1[, wheel2, wheel3]) -- declared as such so koffi uses the
// platform's real variadic calling convention. Apple Silicon passes variadic args on the
// stack even when a fixed-arity call would use registers; getting this wrong silently
// reads garbage instead of the scroll amount.
const CGEventCreateScrollWheelEvent = CG.func("void *CGEventCreateScrollWheelEvent(void *source, uint32_t units, uint32_t wheelCount, ...)");
...
type DarwinEvent =
	...
	| { kind: "mouseWheel"; notches: number };
...
export function mouseWheel(notches: number): unknown {
	const event: DarwinEvent = { kind: "mouseWheel", notches };
	return event;
}
	...
	for (const raw of events) {
		...
		switch (ev.kind) {
			...
			case "mouseMove":
				...
				break;
			case "mouseWheel":
				cgEvent = CGEventCreateScrollWheelEvent(null, kCGScrollEventUnitLine, 1, "int32_t", Math.round(ev.notches));
				break;
		}
