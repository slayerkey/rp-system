...
- HDR: live state plus On / Off / Toggle on Windows 11 24H2+ (build 26100+). PackRat reads the real Windows build through RtlGetVersion and uses the separated Advanced Color Info 2 / HDR State path plus the actual active color mode. Older Windows builds deliberately report HDR as unavailable and perform no legacy Advanced Color write because that older control is ambiguous between HDR and SDR WCG/automatic color management. Native packet sizes are asserted at backend startup so an interop layout mismatch fails closed.
