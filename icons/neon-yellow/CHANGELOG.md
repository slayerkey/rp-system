# Neon Yellow Icons

## 1.0.0 (2026-08-04)

Initial release. Every Stream Deck model, Windows and macOS.

- 500+ icons in the Neon Yellow colorway, PNG at 144x144.
- Streaming, editing, media, system, UI, symbols and meeting controls.
- Text keys for A to Z, 0 to 99 and F1 to F24, plus short labels like LIVE, REC and MUTE.
- 50 brand logos drawn in the same style as the rest of the pack.

Built by ../ratpack-icons/build_pack.py from the Neon-Yellow colorway.
